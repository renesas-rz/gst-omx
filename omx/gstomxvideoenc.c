...
gst_omx_video_enc_set_to_idle (GstOMXVideoEnc * self)
{
  ...
  if (self->input_dmabuf) {
    if (!gst_omx_video_enc_set_use_buffer (self))
      return FALSE;
  } else if (!gst_omx_video_enc_allocate_in_buffers (self)) {
    return FALSE;
  }
#else
...
gst_omx_video_enc_enable (GstOMXVideoEnc * self, GstBuffer * input)
{
...
#elif defined(USE_RZ_DMABUF_IMPORT)
  ...
  GST_WARNING_OBJECT (self, "Setting ports to flushing = %d", FALSE);
    ...
    GstVideoCodecFrame * frame)
{
  ...
  while (acq_ret != GST_OMX_ACQUIRE_BUFFER_OK) {
    ...
    if (self->input_dmabuf) {
      ...
      if (!gst_omx_rz_compare_buffers (buf, frame->input_buffer)) {
