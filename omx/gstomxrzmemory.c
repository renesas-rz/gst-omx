...
#define GST_OMX_RZ_MEMORY_QUARK gst_omx_rz_memory_quark ()
GQuark
gst_omx_rz_memory_quark (void)
{
  static GQuark quark = 0;

  if (quark == 0)
    quark = g_quark_from_static_string ("GstOMXRZMemory");

  return quark;
}

static void
gst_omx_rz_mem_export_destroy (GstOMXRZMemory * rz_mem)
{
  mmngr_export_end_in_user_ext (rz_mem->dmabuf_id);
  g_slice_free (GstOMXRZMemory, rz_mem);
}

GstMemory *
gst_omx_rz_memory_alloc (GstAllocator * dmabuf_allocator,
    GstOMXBuffer * omx_buf)
{
  GstOMXRZMemory *rz_mem;
  GstMemory *mem;
  gint dmabuf_id;
  gint dmabuf_fd;
  ...
  rz_mem = g_slice_new0 (GstOMXRZMemory);
  rz_mem->dmabuf_id = dmabuf_id;
  rz_mem->omx_buf = omx_buf;

  gst_mini_object_set_qdata (GST_MINI_OBJECT (mem),
      GST_OMX_RZ_MEMORY_QUARK, rz_mem,
      (GDestroyNotify) gst_omx_rz_mem_export_destroy);

  return mem;
}

static void
gst_omx_rz_mem_import_destroy (GstOMXRZMemory * rz_mem)
{
  mmngr_import_end_in_user_ext (rz_mem->dmabuf_id);
  g_slice_free (GstOMXRZMemory, rz_mem);
}

static gboolean
gst_omx_rz_import_dmabuf (GstOMXBuffer * buf, GstBuffer * input_buffer)
{
  guint n_mem;
  gint i;
  gint ret;
  ...
  for (i = 0; i < n_mem; i++) {
    ...
    gst_mini_object_set_qdata (GST_MINI_OBJECT (mem),
        GST_OMX_RZ_MEMORY_QUARK, rz_mem,
        (GDestroyNotify) gst_omx_rz_mem_import_destroy);
  }
...
gst_omx_rz_compare_buffers (GstOMXBuffer * buf, GstBuffer * input_buffer)
{
  ...
  rz_mem =
      ...
      GST_OMX_RZ_MEMORY_QUARK);

  if (!rz_mem) {
    if (!gst_omx_rz_import_dmabuf (buf, input_buffer)) {
